...
export default function Dashboard() {
  ...
  useEffect(() => {
    seedDatabase('project-mgmt');
    loadData();
  }, []);

  async function loadData() {
    const items = await getAll('tasks');
  ...
  return (
    <div className="p-6">
      <PageHeader title={dashContent.heading || "Dashboard"} subtitle={dashContent.subheading || "Overview of your project manager"} />
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard label="Active Projects" value={stats[0]} icon="📁" trend={{ value: '2 new', positive: true }} />
        <StatCard label="Total Tasks" value={stats[1]} icon="✅" />
        <StatCard label="In Progress" value={stats[2]} icon="🔄" />
        <StatCard label="Completed" value={stats[3]} icon="🏆" />
      </div>
        ...
        <div className="space-y-3">
          {recent.map((item) => (
            <div key={item.id as string} className="flex items-center justify-between py-2 border-b border-border/50 last:border-0">
              ...
              <p className="text-xs text-muted">{item.project_name as string}</p>
              </div>
            </div>
          ))}
        </div>
